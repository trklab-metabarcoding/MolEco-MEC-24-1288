...
```{r}
emblp6 <- read.csv("../data/20240710_P6_set_1.tab.csv")
nrow(emblp6)
length(unique(emblp6$sequence)) # number of unique sequences 
#levels(factor(emblp6$family_name)) # uncomment and run to print out all the family names
length(unique(emblp6$family_name))
157020/5324 # fold difference between number in BOLD vs EMBL
emblp6_nofamily <- subset(emblp6, family_name == "")
...
length(unique(emblp6$family_name))
#unique(emblp6$family_name) # uncomment to pringt out all the family names
...
write.csv(ds_s2, "../data/Dataset_S2.csv")
```

...
head(table(combtab$family_name, combtab$Multiple.markers), 2)
...
combtabITScount <- tapply(combtab$ITS2, combtab$Family, function(x) length(which(x=="ITS")))
...
head(sort(table(droplevels(combtab[which(combtab$Family %in% nomatchnames),]$Family)), decreasing=T), 2)

sum(sort(table(droplevels(combtab[which(combtab$Family %in% nomatchnames),]$Family)), decreasing=T))

sum(sort(table(droplevels(combtab[which(combtab$Family %in% nomatchnames),]$Family)), decreasing=T))/nrow(combtab)

median(sort(table(droplevels(combtab[which(combtab$Family %in% nomatchnames),]$Family)), decreasing=T))

# Number of specimens not identified to family
length(which(combtab$Family == ""))
```
